...
## Steam popup workaround (Niri)

This branch contains a temporary workaround for Steam dropdown/hover popups closing immediately when Steam is run through xwayland-satellite under Niri.

The regression was bisected to commit `3273a0fccd71da21c6362c74f3b1d1c0a89ff3ba` ("Fix: unity editor add component pop up (#397)").

On current main, disabling forced WM focus for newly configured popups fixes Steam menus such as View/Friends and the Store/Library/Community hover popups.

The workaround changes this condition in `src/server/event.rs`:
...
The same Steam build was also tested under Labwc using native Xwayland, where the menus work normally.

**Important:** this is a workaround, not a complete upstream fix. It disables the popup WM-focus path added for Unity Editor, so Unity Editor popup keyboard input may regress.
...
It can be placed in a systemd user unit directory (i.e. `$XDG_CONFIG_HOME/systemd/user` or `/etc/systemd/user`), and be launched and enabled with:

```bash
systemctl --user enable --now xwayland-satellite
```

It will be started when the `graphical-session.target` is reached, which is likely after your compositor is started if it supports systemd.
...
You can view [Niri's implementation of this integration](https://github.com/YaLTeR/niri/pull/1728/files) for understanding how it should work.
...
Although `satellite` cannot automatically do this, you can configure Java to do this by adding:
